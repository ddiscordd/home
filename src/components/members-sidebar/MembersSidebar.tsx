...
function Group({ title, users, dimmed = false }: { title: string; users: User[]; dimmed?: boolean }) {
  ...
  return (
    <div>
      <p className="px-2 pb-2 text-[11px] font-semibold tracking-wider text-slate-500 uppercase">{title}</p>
        ...
        {users.map((user) => (
          <li key={user.id}>
            <button
              ...
              className="flex w-full items-center gap-2.5 rounded-[var(--radius-md)] px-2 py-1.5 text-left transition-all duration-150 hover:bg-white/[0.04]"
            >
